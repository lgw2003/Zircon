...
namespace Client.Scenes
{
    public sealed class GameScene : DXScene
    {
        ...
        private void CreateItemLabel()
        {
            ...
            if (MouseItem.Info.ItemEffect == ItemEffect.ItemPart)
            {
                label = new DXLabel
                {
                    ...
                    Location = new Point(ItemLabel.DisplayArea.Right, 4),
                    Parent = ItemLabel,
                    Text = $"碎片: {MouseItem.Count}/{displayInfo.PartCount}.",
                };
                ...
                case ItemType.Book:
                    if (MouseItem.Info.Durability > 0)
                    {
                        label = new DXLabel
                        {
                            ...
                            Location = new Point(ItemLabel.DisplayArea.Right, 4),
                            Parent = ItemLabel,
                            Text = $"页: {MouseItem.CurrentDurability}/{MouseItem.MaxDurability}",
                        };
            ...
            if (displayInfo.RequiredClass != RequiredClass.All)
            {
                ...
                label = new DXLabel
                {
                    ...
                    Location = new Point(4, ItemLabel.DisplayArea.Bottom),
                    Parent = ItemLabel,
                    Text = $"需要职业: {description?.Description ?? displayInfo.RequiredClass.ToString()}",
                };
            ...
            if (displayInfo.RequiredAmount > 0)
            {
                ...
                switch (displayInfo.RequiredType)
                {
                    case RequiredType.Level:
                        text = $"需要等级: {MouseItem.Info.RequiredAmount}";
            ...
            if (MouseItem.Info.Durability > 0 && MouseItem.Info.CanRepair && MouseItem.Info.StackSize == 1 && MouseItem.Info.ItemType != ItemType.Book)
            {
                ...
                {
                    label.Text = $"下次特殊修理 {Functions.ToString(MouseItem.NextSpecialRepair - CEnvir.Now, true)}";
                    label.ForeColour = Color.Red;
                }
            ...
            if (NPCItemFragmentBox.IsVisible && MouseItem.CanFragment())
            {
                ...
                label = new DXLabel
                {
                    ...
                    Location = new Point(4, ItemLabel.DisplayArea.Bottom),
                    Parent = ItemLabel,
                    Text = $"拆解: {(MouseItem.Info.Rarity == Rarity.Common ? "初级碎片" : "中级碎片")} x{MouseItem.FragmentCount():#,##0}",
                };
        ...
        private void EquipmentItemInfo()
        {
            ...
            if (MouseItem.Info.Durability > 0)
            {
                label = new DXLabel
                {
                    ...
                    Location = new Point(ItemLabel.DisplayArea.Right, 4),
                    Parent = ItemLabel,
                    Text = $"耐久: {Math.Round(MouseItem.CurrentDurability / 1000M)}/{Math.Round(MouseItem.MaxDurability / 1000M)}",
                };
        ...
        private void CreatePotionLabel()
        {
            ...
            if (MouseItem.Info.Durability > 0)
            {
                label = new DXLabel
                {
                    ...
                    Location = new Point(4, ItemLabel.DisplayArea.Bottom),
                    Parent = ItemLabel,
                    Text = $"冷却CD: {MouseItem.Info.Durability / 1000M:#,##0.#} 秒"
                };
        ...
        public void UseMagic(ClientUserMagic magic)
        {
            ...
            switch (magic.Info.Magic)
            {
                ...
                case MagicType.SweetBrier:
                    ...
                    {
                        ReceiveChat(string.Format("{0} 已就绪", magic.Info.Name), MessageType.Hint);
            ...
            if (CEnvir.Now < magic.NextCast)
            {
                if (CEnvir.Now >= OutputTime)
                {
                    ...
                    ReceiveChat(string.Format("无法使用技能 {0}, 技能还在冷却中.", magic.Info.Name), MessageType.Hint);
                }
                return;
            }
            ...
            switch (magic.Info.Magic)
            {
                case MagicType.Cloak:
                    ...
                    if (CEnvir.Now < User.CombatTime.AddSeconds(10))
                    {
                        ...
                        {
                            OutputTime = CEnvir.Now.AddSeconds(1);
                            ReceiveChat(string.Format("你还处于战斗状态 无法使用技能 {0}", magic.Info.Name), MessageType.Hint);
                        }
                        return;
                    }
                    ...
                    if (User.Stats[Stat.Health] * magic.Cost / 1000 >= User.CurrentHP || User.CurrentHP < User.Stats[Stat.Health] / 10)
                    {
                        ...
                        {
                            OutputTime = CEnvir.Now.AddSeconds(1);
                            ReceiveChat(string.Format("无法使用技能 {0}, 你没有足够的生命值", magic.Info.Name), MessageType.Hint);
                        }
                        return;
                    }
                    break;
                case MagicType.DarkConversion:
                    ...
                    if (magic.Cost > User.CurrentMP)
                    {
                        ...
                        {
                            OutputTime = CEnvir.Now.AddSeconds(1);
                            ReceiveChat(string.Format("无法使用技能 {0}, 你没有足够的魔法值", magic.Info.Name), MessageType.Hint);
                        }
                        return;
                    }
                    break;
            ...
            switch (magic.Info.Magic)
            {
                ...
                case MagicType.DanceOfSwallow:
                    ...
                    if (!Functions.InRange(target.CurrentLocation, User.CurrentLocation, Globals.MagicRange))
                    {
                        ...
                        OutputTime = CEnvir.Now.AddSeconds(1);
                        ReceiveChat(string.Format("无法使用技能 {0}, 你的目标太远了", magic.Info.Name), MessageType.Hint);
                        return;
                    }
        ...
        public void UseMagic(SpellKey key)
        {
            ...
            //施毒术 自动换毒
            if (magic.Info.Magic == MagicType.PoisonDust || magic.Info.Magic == MagicType.AugmentPoisonDust)
            {
                ReceiveChat("施毒术", MessageType.Hint);
                ClientUserItem equipmentPoison = Equipment[(int)EquipmentSlot.Poison];
                DXItemCell greenPoisonCell = InventoryBox.Grid.Grid.FirstOrDefault(x => x.Item?.Info.ItemName == "绿毒");
                DXItemCell redPoisonCell = InventoryBox.Grid.Grid.FirstOrDefault(x => x.Item?.Info.ItemName == "红毒");
                if (equipmentPoison != null)
                {
                    if (equipmentPoison.Info.ItemName == "绿毒")
                    {
                        if (redPoisonCell != null)
                        {
                            redPoisonCell.UseItem();
                        }
                    }
                    else
                    {
                        if (greenPoisonCell != null)
                        {
                            greenPoisonCell.UseItem();
                        }
                    }
        ...
        public string GetTaskText(QuestTask task, ClientUserQuest userQuest)
        {
            ...
            switch (task.Task)
            {
                ...
                case QuestTaskType.Region:
                    builder.AppendFormat("位于 {1} 的 {0} ", task.RegionParameter?.Description, task.RegionParameter?.Map.Description);
                    break;
            }
            ...
            if (string.IsNullOrEmpty(task.MobDescription))
            {
                ...
                for (int i = 0; i < task.MonsterDetails.Count; i++)
                {
                    ...
                    if (monster.Map != null)
                        builder.AppendFormat(" 的 {0}", monster.Map.Description);
                }
            }
            else
